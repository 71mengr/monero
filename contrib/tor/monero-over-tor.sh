...
if test -z "$uzoqamd"
then
  echo "uzoqamd not found"
  exit 1
fi
echo "Found: $uzoqamd"

TORDIR="$DIR/uzoqam-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting uzoqamd"
  tail -n 400 "$HOME/.bituzoqam/bituzoqam.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Uzoqam\ \'" | tail -n 20
  exit 1
fi
