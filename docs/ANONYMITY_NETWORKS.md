...
is made or until `uzoqamd` is shutdown and restarted with only public
connections enabled.
...
`--tx-proxy` which tells `uzoqamd` the ip/port of a socks proxy provided by a
...
which tells `uzoqamd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `uzoqamd` the inbound address, network
...
which tells `uzoqamd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `uzoqamd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `uzoqamd` localhost port 30000.
...
`uzoqamd` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 40000 for mainnet) instead of the p2p port:
...
If a user only runs `uzoqamd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `uzoqamd` as often as possible to conceal when transactions are being sent.
...
`uzoqamd` currently selects two outgoing connections every 5 minutes for
...
`uzoqamd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `uzoqamd` should likely disconnect
